...
//生成节假日提示信息地图
    ...
    Calendar.prototype = {
        ...
        _init: function (config) {
            ...
            this.uid = config.uid || 1;

            Calendar.calendarList.push( {
                uid : config.uid,
                cal : this
            } );
            ...
            this.triggerNode = this.triggerNodeFn( config.id );
            ...
            var calendarId = this.triggerNode.getAttribute('calendar-id');
            if( calendarId ){
                for (var i = 0, len = Calendar.calendarList.length; i < len; i++) {
                    if ( Calendar.calendarList[i].id == calendarId ) {
                        var cal = Calendar.calendarList[i].cal;
                        //cal._addEvent()._setPos(this.triggerNodeFn(this.id));
                        //cal.revise = {top: 2, left: 0};
                        cal.dateRange({
                            mindate : this.range.mindate,
                            maxdate : this.range.maxdate
                        });
                        return Calendar.calendarList[i].cal;
                    }
                }
            }else{
                this.triggerNode.setAttribute('calendar-id', this.id);
                Calendar.calendarList.push({
                    id : this.id,
                    cal : this
                });
         ...
         * @returns {HTMLElement}
         */
        triggerNodeFn : function( id ){
            if( typeof id == 'string' ){
                return _CAL.$("#" + id.replace(/^#/, ""));
            }
            return id;
        },
        /**
        ...
        dateRange : function( dateRange ){
             ...
             if( days instanceof Array && days.length ){
                 for( var i = 0, len = days.length; i < len; i++ ){
                     var oA = days[i], saveDay = oA["data-date"];

                     var iCur = _CAL.formatIntDate( saveDay );
                     ...
                     if (dateRange.mindate) {
                         iCur < (_CAL.formatIntDate( dateRange.mindate )) && (oA.className = "disabled");

                     }

                     if (dateRange.maxdate) {
                         iCur > (_CAL.formatIntDate( dateRange.maxdate )) && (oA.className = "disabled");
                     }

                     //指定了日期则增加选中状态
                     this.selectDate == saveDay && ((oA.children[0] || oA).className = "selected");

                     //开始日期
                     this.startDate == saveDay && ((oA.children[0] || oA).className = "start-date");

                     //开始日期
                     this.endDate == saveDay && ((oA.children[0] || oA).className = "end-date");
            ...
            _CAL.on(obj, "click", function (e) {
                ...
                if (oTarget.tagName.toUpperCase() === "A" && oTarget.className != "disabled") {
                    ...
                    if (that.triggerNode.tagName.toUpperCase() === "INPUT") {
                        if( that.isMulti ){
                            if( dateRex.test( that.triggerNode.value )){
                                if( that.triggerNode.value.indexOf(oTarget["data-date"]) === -1 )
                                that.triggerNode.value = oTarget["data-date"] + ',' + that.triggerNode.value;
                            }else{
                ...
                that.onselect && that.onselect(oTarget["data-date"]);
            });
            //日历为弹出显示时添加事件
            if (this.isPopup) {
    ...
    huiyan['Calendar'] = function( config ){
